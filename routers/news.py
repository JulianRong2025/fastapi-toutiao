...
async def get_news_detail(db: AsyncSession = Depends(get_db), news_id: int = Query(..., alias="id")):
    ...
    views_res = await news_cache.increase_news_views(db, news_detail.id)  # 浏览量+1
    if not views_res:
        raise HTTPException(status_code=404, detail="更新浏览量失败")
    
    releated_news = await news_cache.get_related_news(db, news_detail.category_id, news_detail.id)
